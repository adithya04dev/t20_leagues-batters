...
class Batsman():
    ...
    def create_df(self,leagues,player,overs,BowlingType,Season):
            ...
            run = int(self.df.loc[(self.df["batter"] == player) & (self.df["LeagueName"].isin(leagues))  & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))].batsman_run.sum())
            balls=len(self.df.loc[(self.df['extra_type']!="wides") & (self.df["LeagueName"].isin(leagues)) & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season)) & (self.df['extra_type']!="noballs") & (self.df['batter'] == player) ] )
            out = len(   self.df.loc[(self.df["player_out"] == player) & (self.df["LeagueName"].isin(leagues)) & (self.df["BowlingType"].isin(BowlingType)) & ~(self.df["dismissal_kind"].isin(dis))  & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
            boundary = len(self.df.loc[(self.df["batter"] == player) & (self.df["LeagueName"].isin(leagues)) &  ((self.df["batsman_run"] == 4) | (self.df["batsman_run"] == 6)  ) & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
            dots=len(self.df.loc[(self.df["batter"] == player) & (self.df["LeagueName"].isin(leagues)) & (self.df["Extras_Run"]==0) & (self.df["batsman_run"]==0) & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season))  ] )
    ...
    def combined(self):
                return self.comb_df    
# bat=Batsman(df)
# # result=bat.calculate(['SA20'],'H Klaasen',[1,2,3],["Spin"],[2023]) 
# # result1=bat.overall()
# # print(result1['strike_rate'])

# with open("C:/Users/adith/Documents/ds/all_t20_app/individual/batting/batting.pkl", 'wb') as f:
#     pickle.dump(bat, f)
